...
class Separator:
    def __init__(
        ...
        self.audio_file_base = os.path.splitext(
        ...
        if self.normalization_enabled:
            self.logger.debug(
                f"Normalization enabled, waveform will be normalized to max amplitude of 1.0 to avoid clipping.")
        else:
            self.logger.debug(
                f"Normalization disabled, waveform will not be normalized.")

        self.denoise_enabled = denoise_enabled
        if self.denoise_enabled:
            self.logger.debug(
                f"Denoising enabled, model will be run twice to reduce noise in output audio.")
        else:
            self.logger.debug(
        ...
        if output_single_stem is not None:
            if output_single_stem.lower() not in {"instrumental", "vocals"}:
                raise Exception(
                    "output_single_stem must be either 'instrumental' or 'vocals'")
            self.logger.debug(
        ...
        if self.use_cuda:
            self.logger.debug(
                "CUDA requested, checking Torch version and CUDA status")
            ...
            if cuda_available:
                ...
                raise Exception(
                    "CUDA requested but not available with current Torch installation. Do you have an Nvidia GPU?")

        elif self.use_coreml:
            self.logger.debug(
                "Apple Silicon CoreML requested, checking Torch version")
            self.logger.debug(f"Torch version: {str(torch.__version__)}")

            mps_available = hasattr(
                torch.backends, "mps") and torch.backends.mps.is_available()
            self.logger.debug(
                f"Is Apple Silicon CoreML MPS available? {str(mps_available)}")
    ...
    def separate(self):
        model_path = os.path.join(
            self.model_file_dir, f"{self.model_name}.onnx")
        if not os.path.isfile(model_path):
            self.logger.debug(
                f"Model not found at path {model_path}, downloading...")
        ...
        if not os.path.isfile(model_data_path):
            self.logger.debug(
                f"Model data not found at path {model_data_path}, downloading...")
        ...
        mix, raw_mix, samplerate = prepare_mix(
            self.audio_file_path, self.chunks, self.margin, mdx_net_cut=mdx_net_cut)
        ...
        if not isinstance(self.primary_source, np.ndarray):
            self.primary_source = spec_utils.normalize(
                self.logger, source, self.normalization_enabled).T

        if not isinstance(self.secondary_source, np.ndarray):
            raw_mix = self.demix_base(raw_mix, is_match_mix=True)[
                0] if mdx_net_cut else raw_mix
        ...
        if not self.output_single_stem or self.output_single_stem.lower() == self.primary_stem.lower():
            ...
            self.write_audio(self.primary_stem_path,
                             self.primary_source, samplerate)
        ...
        if not self.output_single_stem or self.output_single_stem.lower() == self.secondary_stem.lower():
            ...
            self.write_audio(self.secondary_stem_path,
                             self.secondary_source, samplerate)
    ...
    def write_audio(self, stem_path, stem_source, samplerate):
        ...
        sf.write(stem_path, stem_source, samplerate,
                 subtype=self.output_subtype, format=self.output_format)
    ...
    def initialize_model_settings(self):
        ...
        self.window = torch.hann_window(
            window_length=self.n_fft, periodic=False).to(self.device)
        self.freq_pad = torch.zeros(
            [1, self.dim_c, self.n_bins - self.dim_f, self.dim_t]).to(self.device)
    ...
    def initialize_mix(self, mix, is_ckpt=False):
        if is_ckpt:
            ...
            mix_waves = [mixture[:, i * self.gen_size: i *
                                 self.gen_size + self.chunk_size] for i in range(num_chunks)]
        else:
            mix_waves = []
            n_sample = mix.shape[1]
            pad = self.gen_size - n_sample % self.gen_size
            mix_p = np.concatenate((np.zeros((2, self.trim)), mix, np.zeros(
                (2, pad)), np.zeros((2, self.trim))), 1)
            i = 0
    ...
    def demix_base(self, mix, is_ckpt=False, is_match_mix=False):
        ...
        for slice in mix:
            ...
            with torch.no_grad():
                ...
                tar_waves_ = np.vstack(tar_waves_)[
                    :, :, self.trim: -self.trim] if is_ckpt else tar_waves_
                tar_waves = np.concatenate(tar_waves_, axis=-1)[:, :pad]
                start = 0 if slice == 0 else self.margin
                end = None if slice == list(
                    mix.keys())[::-1][0] or self.margin == 0 else -self.margin
    ...
    def run_model(self, mix, is_ckpt=False, is_match_mix=False):
        ...
        if is_match_mix:
            ...
            spec_pred = -self.model_run(-spek) * 0.5 + self.model_run(
                spek) * 0.5 if self.denoise_enabled else self.model_run(spek)
    ...
    def istft(self, x, freq_pad=None):
        freq_pad = self.freq_pad.repeat(
            [x.shape[0], 1, 1, 1]) if freq_pad is None else freq_pad
