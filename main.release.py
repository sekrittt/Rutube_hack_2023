...
def get_audio(*, input_path: str, output_dir: str) -> Optional[str]:
    ...
    try:
        ...
        subprocess.run(['ffmpeg', '-i', input_path, '-vn', '-acodec',
                        'pcm_s16le', '-ar', '44100', '-ac', '1', output_path, '-y'], check=True)
        print(f"Aудио успешно извлечено: {output_path}")
        return output_path
...
def split_audio(*, input_path: str, output_path: str) -> Optional[tuple[str, str]]:
    ...
    try:
        separator = Separator(input_path, model_name='UVR_MDXNET_KARA_2', use_cuda=True if device == 'cuda' else False,
                              model_file_dir=path_to('models', 'asm'), output_dir=output_path)
        ...
        print('Аудио-файл разделён успешно!')
...
def synthesize_speech(*, text: str, file_name: str, output_path: str, lang: Langs) -> Optional[str]:
    ...
    try:
        ...
        speaker = Speaker(model_id=f"v3_{lang}", language=lang,  # type: ignore
                          speaker=f"{lang}_1", device=device)  # type: ignore
        ...
        print('Речь успешно синтезирована!')
...
def sync_non_speech_fragments(*, origin_file: str, target_file: str) -> bool:
    ...
    try:
        ...
        print('Фрагменты успешно синхронизированы!')
        return True

    except Exception as e:
        print(f"Ошибка при синхронизации фрагментов: {e}")
        return False
...
def combine_audio_files(*, background_file: str, voice_file: str, output_path: str) -> Optional[str]:
    ...
    try:
        ...
        print('Аудио-файлы успешно объединены!')
        return combined_file_path

    except Exception as e:
        print(f"Ошибка при объединении аудио-файлов: {e}")
        return None
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--use-cuda', type=bool, required=False, default=False, help="использовать cuda или нет (по умолчанию - В зависимости от установки)")
    ...
    print(
        f"Работы программы завершена. Результат выполнения программы: \n\t{message}")
